...
resource "aws_iam_policy" "rodin_policy_lambda" {
  ...
  name = "${var.namespace}_policy_${var.env}"
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:dynamodb:${var.region}:${var.account_id}:table/tate_*_${var.env}"
    }
   ]
}
EOF
...
resource "aws_iam_role" "rodin_role_lambda" {
  ...
  name = "${var.namespace}_role_lambda_${var.env}"
